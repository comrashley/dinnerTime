...
const getValidTimeData = (options = {}) => {
  const {tz, time, timeMode, useTz = true, meridiem = null} = options
  ...
  if (time || !useTz) {
    time24 = (validTime
      ...
      : moment().format(format24)
    ).split(/:/)
    time12 = (validTime
      ? formatTime.format(format12)
      : moment().format(format12)
    ).split(/:/)
  } else {
    time24 = (validTime
...
const getValidTimeString = (time, meridiem) => {
  if (is.string(time)) {
    let validTime =
      time && time.indexOf(':').length >= 0
        ? time
            .split(/:/)
            .map(t => getValidateTime(t))
            .join(':')
        : time
...
const tzNames = (() => {
  ...
  const tznames = moment.tz
    .names()
    .filter(name => name.indexOf('/') >= 0)
    .filter(name => !scrubbedPrefixes.indexOf(name.split('/')[0]) >= 0)
    .filter(
      name => !scrubbedSuffixes.indexOf(name.split('/').slice(-1)[0]) >= 0
    )
...
const tzCities = tzNames
  .map(
    name =>
      ['Canada', 'Mexico', 'US'].indexOf(name.split('/')[0]) >= 0
        ? name
